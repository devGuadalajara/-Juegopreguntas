...
const dbname = "juegoDB";
const dbcollection = "juegos"
...
io.on('connection', (socket) => {
        ...
        MongoClient.connect(url, function(err, db) {
            ...
            dbo.collection(dbcollection).find(query).toArray(function(err, result){
                ...
                if(result[0] !== undefined){
                    ...
                    console.log('Contraseña del juego: ', game.pin); 
    ...
    socket.on('requestDbNames', function(){
        ...
        MongoClient.connect(url, function(err, db){
            ...
            var dbo = db.db(dbname);
            dbo.collection(dbcollection).find().toArray(function(err, res) {
                if (err) throw err;
    ...
    socket.on('newQuiz', function(data){
        MongoClient.connect(url, function(err, db){
            ...
            dbo.collection(dbcollection).find({}).toArray(function(err, result){
                ...
                var game = data;
                dbo.collection(dbcollection).insertOne(game, function(err, res) {
                    if (err) throw err;
                    db.close();
                });
                db.close();
